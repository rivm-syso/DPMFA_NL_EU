FunPrepStatsALL <- function(Clothing_data_all, # Full tibble/data frame with DPMFA output from PrepData
  ...
  ClothingYearSelect <- c(names(Clothing_data_all)[!names(Clothing_data_all) %in% c(names(Clothing_data_all) |> as.numeric() |> na.omit())],SelectYear)
  return(
    Clothing_data_all |>
      ...
      ungroup() |> group_by(Scale,data_source) |> 
