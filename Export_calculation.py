...
pathtoDB = os.path.join(mainfolder,"input", "NL", "NL.db")
connection = sqlite3.connect(pathtoDB)
cursor = connection.cursor()

cursor.execute("DELETE FROM transfercoefficients WHERE value = 0")
...
# check if some compartments are empty, if yes remove to avoid bugs (normalization of zero TCs does not work)
    ...
    while len(comptoanalyze) != 0:
        ...
        for startcomp in comploop:
            ...
            for endcomp in dest:
                ...
                # check if there are any non-zero datapoints. If so, mark comp as True (no cleanout)
...
comporder = pd.read_excel(file, sheet_name="Input_compartment_order", header=None, usecols=[0]).squeeze().tolist()

#compname = 'Transport of primary plastics'
...
for compname in comporder:
    ...
    # if some input values are negative, need to do the export calculation
        ...
        simulator.runSimulation() # This is where the error occurs
